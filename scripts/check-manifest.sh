...
#      ship.
#
#   3. LEDGER      check-coverage.sh treats the manifest as the record of what
#      the app tree owns. A dangling entry (no such file) quietly widens what
#      that guard considers accounted for.
#
# Unlike check-coverage.sh this needs no external reference, so it does not
# decay when the pre-split fork freezes. See docs/coverage-guards.md.
