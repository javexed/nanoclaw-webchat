...
(1,900+ tests), re-proven by CI on every push. `scripts/check-coverage.sh`
guarantees no webchat file is silently dropped: every file that differs from
the reference tree must be accounted for by the app tree, a patch, or a
