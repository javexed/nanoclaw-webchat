...
afterEach(async () => {
  ...
  // Scoped to the one draft this file writes, NOT the whole directory. Test
  // files run in parallel workers against a shared cwd/data, so removing all of
  // data/skill-drafts here also deletes drafts belonging to whichever file is
  // mid-test elsewhere. skill-draft-keep.test.ts is the exposed one: its async
  // keep reads the body after answering 202, so losing the directory in that
  // window turns a keep into a silent 410 'Draft body missing'.
