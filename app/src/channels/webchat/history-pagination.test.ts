...
describe('getWebchatMessagesBeforeId', () => {
  it('returns the messages immediately before the anchor, oldest-to-newest', async () => {
    const page = await getWebchatMessagesBeforeId('room-1', 'm6', 3);
    expect(page.map((m) => m.id)).toEqual(['m3', 'm4', 'm5']); // the 3 just before m6, ascending
  });

  it('is scoped to the room (no cross-room leakage)', async () => {
    const page = await getWebchatMessagesBeforeId('room-1', 'm6', 50);
  ...
  it('paginates: chaining before_id walks backwards to the start', async () => {
    ...
    const newest = await getWebchatMessages('room-1', 4); // m7..m10
    expect(newest.map((m) => m.id)).toEqual(['m7', 'm8', 'm9', 'm10']);
    const older1 = await getWebchatMessagesBeforeId('room-1', 'm7', 4); // m3..m6
