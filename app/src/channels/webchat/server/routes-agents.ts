...
import { injectSessionCommand } from './agent-wiring.js';
 ...
 * (and this panel) needs to know `$SERVICE_API_KEY` exists, and neither needs to
...
export async function assignAgentModelHandler(
  ...
  const cleared = priorModelId !== (current?.id ?? null) ? await clearTranscriptHarnessSessions(agentGroupId) : 0;
  ...
  return json(res, 200, { ok: true, model: current, reachability, clearedSessions: cleared });
}

/**
 * Harnesses whose continuation is a REPLAYED LOCAL TRANSCRIPT: the next turn
 * re-sends the whole prior conversation to the model. Swap the model under one
...
/**
 * Clear the live sessions of a group whose harness replays transcripts, so a
 * model change starts the new model on a clean history. Returns how many were
 * cleared. Best-effort per session: a session whose inbound.db is missing (never
 * spawned) is skipped, and one failure never blocks the rest or the reassignment
 * itself — the model is already written by the time this runs.
 */
async function clearTranscriptHarnessSessions(agentGroupId: string): Promise<number> {
  const cfg = await getContainerConfig(agentGroupId);
  const provider = cfg?.provider ?? 'claude';
  if (!TRANSCRIPT_REPLAY_HARNESSES.has(provider)) return 0;
  let cleared = 0;
  for (const session of await getSessionsByAgentGroup(agentGroupId)) {
    try {
      injectSessionCommand(agentGroupId, session.id, '/clear');
      cleared++;
    } catch {
      /* never spawned, or a torn-down session — nothing to clear */
    }
  }
  if (cleared > 0) {
    log.info('Cleared transcript-harness sessions after a model change', { agentGroupId, provider, cleared });
  }
  return cleared;
}
