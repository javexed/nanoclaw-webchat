...
async function rToolSecrets(ctx: RouteCtx, _m: RegExpMatchArray): Promise<void> {
  ...
  // Shared scopes (system / whole agent) stay admin-only. A PERSONAL
  // credential is different: it must be entered by its owner — an admin doing
  // it on someone's behalf would have to handle that person's token, which is
  // precisely what per-user credentials exist to prevent. So anyone may manage
  // their OWN, and nobody may manage anyone else's.
  const isSelfScope = scope.kind === 'user' && scope.userId === userId;
  if (!isSelfScope && !isOwner(userId) && !isGlobalAdmin(userId)) return json(res, 403, { error: 'Forbidden' });
  try {
    if (method === 'GET') {
    ...
    const raw = await readJsonBody(req, res);
    if (raw === null) return;
    const body = JSON.parse(raw) as { value?: string; hostPattern?: string };
    ...
    // Injection scheme is inferred from the host (injectionForHost) rather
    // than asked for — the operator should not need to know an API's auth
    // header by heart, and one credential per host is the sane model.
    const created = await createToolSecret(realOnecliAdmin, scope, hostPattern, value);
    ...
    const safe =
      /^(No OneCLI agent|Isolate this agent|No model credential|This person has not|A credential for)/.test(msg);
