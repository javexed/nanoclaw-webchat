...
export async function probeTailscaleHealth(run: TailscaleProbeRunner = defaultProbeRunner): Promise<void> {
  ...
  tailscaleProbeInFlight = (async () => {
    ...
    if (!ok) {
      // Log only on a state change (or first probe) so a background re-probe
      // loop can't spam the log while tailscale is simply absent.
      if (wasHealthy !== false) {
        if (notInstalled) {
      ...
      if (wasHealthy !== true) log.info('Webchat: tailscale detected — tailnet sign-in is now available');
...
export function refreshTailscaleHealth(): void {
  if (!TAILSCALE_ENABLED || tailscaleProbeInFlight) return;
