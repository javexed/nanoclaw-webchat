...
registerFeatureInstall('tts', {
  ...
  steps: (root) => [
    { run: ['bash', [ttsInstallerPath(root)]], label: 'Installing the Kokoro TTS container' },
    ...
    return [
      {
        ...
        label: 'Installing the whisper.cpp container',
      },
      { call: () => activateSttEnv(root), label: 'activate (no restart needed)' },
    ];
  },
...
registerFeatureInstall('tailscale', {
  ...
  steps: () => [
    ...
    // never-completed sign-in from hanging the chain forever.
    {
      ...
      label: 'Signing in — open the URL in the log',
    },
  ],
});
...
registerFeatureInstall<{ token?: string }>('cloudflared', {
  ...
  steps: (_root, args) =>
    args?.token === undefined
      ? [{ run: ['bash', ['-c', CLOUDFLARED_PKG_INSTALL]], label: 'Installing cloudflared from its signed repo' }]
      : [
          {
            run: [
              'bash',
            ...
            label: 'Registering the tunnel connector service',
          },
        ],
});
...
registerFeatureInstall('ollama', {
  ...
  steps: () => [{ run: ['sh', ['-c', OLLAMA_INSTALL_SCRIPT]], label: 'Downloading and installing Ollama' }],
});
...
/**
 * Auto-create routes for capabilities the current roster can serve but no route
 * covers yet — the "add a model → a route appears" behavior. Reuses
 * getRouteSuggestions (roster scored against the capability catalog) and appends
 * the winners to the primary router via the canonical merge. Idempotent; returns
 * how many routes were added. Runs as the final step of the routing install and
 * every roster-refresh (after bind-routes rebinds the existing ones).
 */
export async function applyRouteSuggestions(root = process.cwd()): Promise<number> {
  const cfg = readRoutesConfig(root);
  if (!cfg) return 0;
  const suggestions = await getRouteSuggestions(root);
  if (suggestions.length === 0) return 0;
  const current = primaryRouter(cfg).routes;
  const covered = new Set(current.map((r) => r.name));
  const additions = suggestions
    .filter((s) => !covered.has(s.capability))
    .map((s) => ({ name: s.capability, description: s.description, model: s.model }));
  if (additions.length === 0) return 0;
  const merged = mergeRoutesUpdate(cfg, { routes: [...current, ...additions] }, primaryRouterName(cfg));
  writeRoutesConfig(merged, root);
