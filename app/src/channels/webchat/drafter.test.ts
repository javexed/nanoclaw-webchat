...
describe('draftAgent — error mapping', () => {
  ...
  it('maps OneCLI 401 to a 503 with the secret-mode hint', async () => {
    ...
    const err = await draftAgent('hi').catch((e) => e);
    expect(err.status).toBe(503);
    // Assert the ACTIONABLE parts, not a phrase: the required mode and the
    // command that fixes it. The previous assertion pinned wording that turned
    // out to be a wrong diagnosis, so it survived being corrected elsewhere.
    expect(String(err.message)).toContain('all` secret mode');
    expect(String(err.message)).toContain('set-secret-mode');
  });
