...
describe('recordWebchatApproval idempotency', () => {
  it('composite key: same (approval_id, platform_id) is a no-op; a new inbox adds a row (fan-out)', async () => {
    ...
    const before = (await getDb().get(
      `SELECT recorded_at FROM webchat_approvals_index WHERE approval_id = ? AND platform_id = ?`,
      'a-dup',
      platformId,
    )) as { recorded_at: number } | undefined;
    ...
    const rows = (await getDb().all(
      ...
      'a-dup',
    )) as { platform_id: string; recorded_at: number }[];
