...
describe('deskState', () => {
  it('is cold when no container is running, whatever the last event said', async () => {
  ...
  it('is idle when a container is up and its last turn finished', async () => {
    expect(deskState(true, 'done', 5 * MIN)).toBe('idle');
  });

  it('is idle when a container is up but never streamed anything', async () => {
  ...
  it('is working mid-turn, before the ceiling', async () => {
    for (const kind of ['start', 'tool', 'progress', 'reasoning']) {
      expect(deskState(true, kind, 5 * MIN)).toBe('working');
    }
  });

  it('is stuck once a mid-turn desk passes the sweep ceiling', async () => {
    expect(deskState(true, 'tool', STUCK_AFTER_MS + 1)).toBe('stuck');
  });

  it('is still working exactly AT the ceiling, not stuck', async () => {
  ...
  it('is stuck when a container is alive but the host called it stalled', async () => {
  ...
  it('does not call a long-quiet desk stuck when idle age is unknown', async () => {
...
describe('sortDesks', () => {
  ...
  it('puts trouble first, so a 40-desk floor still reads at a glance', async () => {
  ...
  it('breaks ties by how long the desk has been quiet, longest first', async () => {
...
describe('roomFor', () => {
  ...
  it('uses the platform id for webchat, which is what room click-through expects', async () => {
  ...
  it('still names a non-webchat room, but offers no click-through', async () => {
  ...
  it('falls back to the platform id when a group has no name', async () => {
    expect(roomFor({ channel_type: 'webchat', platform_id: 'weight-sensor', name: null }).roomName).toBe(
      'weight-sensor',
    );
  });

  it('reports no room for an agent-shared session', async () => {
