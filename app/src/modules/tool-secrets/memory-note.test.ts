...
describe('credential note placement', () => {
  it('writes an OKF-typed concept file with the hosts, never the values', async () => {
    scaffoldIndex();
  ...
  it('puts a pointer in index.md — the file the session prompt embeds', async () => {
    scaffoldIndex();
    await syncCredentialNote('ag-1', ['git.example.com'], []);
  ...
  it('NEVER creates index.md — that would suppress the container scaffold', async () => {
    await syncCredentialNote('ag-1', ['git.example.com'], []);
  ...
  it('is idempotent — a repeat sync changes nothing', async () => {
    scaffoldIndex();
    await syncCredentialNote('ag-1', ['a.example.com', 'b.example.com'], []);
    const first = fs.readFileSync(indexFile, 'utf-8');
    await syncCredentialNote('ag-1', ['b.example.com', 'a.example.com'], []);
    expect(fs.readFileSync(indexFile, 'utf-8')).toBe(first);
  });

  it('revoking every credential removes both the block and the concept file', async () => {
    scaffoldIndex();
    await syncCredentialNote('ag-1', ['git.example.com'], []);
    await syncCredentialNote('ag-1', [], []);
  ...
  it('caps the inline host list so the always-loaded index stays small', async () => {
    scaffoldIndex();
    const many = Array.from({ length: 20 }, (_, i) => `h${i}.example.com`);
    await syncCredentialNote('ag-1', many, []);
    const idx = fs.readFileSync(indexFile, 'utf-8');
  ...
  it('retires a block left behind in the pre-cutover CLAUDE.local.md', async () => {
    scaffoldIndex();
    ...
    await syncCredentialNote('ag-1', ['new.example.com'], []);
  ...
  it('a missing group folder is a no-op, not a throw', async () => {
    fs.rmSync(groupDir, { recursive: true, force: true });
