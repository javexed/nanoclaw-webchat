...
function addUser(id: string): void {
  getDb().prepare(`INSERT INTO users (id, kind, display_name, created_at) VALUES (?, 'human', ?, ?)`).run(id, id, now());
}
...
describe('handleRouteLearningReview — enrollment and policy', () => {
  ...
  it('falls back to the origin session (workspace credential) for a privileged invoker', async () => {
    ...
    getDb()
      .prepare(`INSERT INTO user_roles (user_id, role, agent_group_id, granted_at) VALUES ('webchat:boss', 'owner', NULL, ?)`)
      .run(now());
