...
describe('agent env store', () => {
  it('round-trips a value and lists it by name', async () => {
    const s = await load();
    s.setAgentEnv('ag-1', 'SABNZBD_API_KEY', 'abc123');
    expect(s.readAgentEnv('ag-1')).toEqual({ SABNZBD_API_KEY: 'abc123' });
    expect(s.listAgentEnvNames('ag-1')).toEqual(['SABNZBD_API_KEY']);
  });
  ...
  it('refuses names that are not env tokens', async () => {
    ...
    for (const ok of ['A', '_X', 'SABNZBD_API_KEY', 'A1_B2']) expect(s.isValidEnvName(ok)).toBe(true);
  });
