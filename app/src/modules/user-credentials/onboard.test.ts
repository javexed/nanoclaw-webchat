...
describe('setWorkspaceDefaultCredential fan-out (re-mint must not orphan selective agents)', () => {
  /** A group whose OneCLI agent is isolated — it only gets what is assigned. */
  async function isolatedGroup(
    admin: OnecliAdmin,
  ...
  it('re-points every selective agent at the NEW secret id', async () => {
    const { admin, agents } = fakeAdmin();
  ...
  it('leaves a member who brought their own credential alone', async () => {
    const { admin, agents } = fakeAdmin();
    await isolatedGroup(admin, agents, 'ag-1');
    await storeUserCredential(admin, 'webchat:alice', 'claude', 'sk-ant-alice', 'api_key');
    await ensureGroupEnrollment(admin, 'webchat:alice', 'ag-1');
