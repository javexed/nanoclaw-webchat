...
def _est_tokens(messages):
    """Rough token estimate for the WHOLE request (~4 chars/token), string and
    parts-list content alike. Drives the context guard; being ~20% off is fine —
    bindings' max_prompt_tokens already halves the window conservatively."""
    chars = 0
...
def _fit_override(router, target, est_tokens):
    ...
    cap = caps.get(target)
    if not isinstance(cap, int) or est_tokens <= cap:
        return target, None
    fitting = [(m, c) for m, c in caps.items() if isinstance(c, int) and c >= est_tokens]
    if fitting:
        best = max(fitting, key=lambda mc: mc[1])[0]
        return best, f"ctx: est {est_tokens} > {cap} of {target}"
    known = [c for c in caps.values() if isinstance(c, int)]
    return None, f"ctx: est {est_tokens} exceeds every local binding (max {max(known, default=0)})"
...
async def _route_live(cfg, router, router_name, data, prompt_text):
    ...
    try:
        ...
        guarded, reason = _fit_override(router, target, est)
        if reason:
        ...
        entry["ctx_guard_error"] = f"{type(e).__name__}: {e}"[:200]
    if target:
        data["model"] = target
