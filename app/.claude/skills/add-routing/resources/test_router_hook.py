...
class EstTokens(unittest.TestCase):
    def test_string_and_parts_content(self):
        msgs = [
            {"role": "system", "content": "x" * 400},
            {"role": "user", "content": [{"type": "text", "text": "y" * 400}]},
        ]
        self.assertEqual(router_hook._est_tokens(msgs), 200)

    def test_empty_and_none(self):
        self.assertEqual(router_hook._est_tokens([]), 0)
        self.assertEqual(router_hook._est_tokens(None), 0)


class FitOverride(unittest.TestCase):
    ROUTER = {
        "routes": [
    ...
    def test_fits_no_override(self):
        target, reason = router_hook._fit_override(self.ROUTER, "small", 1000)
        self.assertEqual(target, "small")
        self.assertIsNone(reason)

    def test_no_metadata_is_inert(self):
        router = {"routes": [{"name": "general", "model": "small"}]}
        target, reason = router_hook._fit_override(router, "small", 999999)
        self.assertEqual(target, "small")
        self.assertIsNone(reason)

    def test_reroutes_to_binding_with_capacity(self):
        target, reason = router_hook._fit_override(self.ROUTER, "small", 4000)
    ...
    def test_non_int_cap_is_inert(self):
        router = {"routes": [{"name": "general", "model": "small", "max_prompt_tokens": "oops"}]}
        target, reason = router_hook._fit_override(router, "small", 999999)
        self.assertEqual(target, "small")
        self.assertIsNone(reason)
