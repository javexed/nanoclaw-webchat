...
#   - escalation.ts: a turn-retry handler (R5) that re-runs a failed turn on
#     the fallback — fresh session, original error preserved when it declines,
#     and CAPPED at NANOCLAW_ESCALATION_CAP consecutive escalations (default 3,
#     reset on the first delivered turn; 0 disables escalation entirely)
...
# ── 2. New files (migration + escalation module) ──────────────────────────
# The escalation logic is a MODULE, not a core patch. It registers a turn-retry
# handler (runner-hooks.ts R5), so the ~154-line poll-loop patch this skill used
# to carry is gone — core owns the retry lifecycle, the module picks the
# provider and owns the spend cap.
...
cp "$HERE/escalation.ts" container/agent-runner/src/escalation.ts
cp "$HERE/escalation.test.ts" container/agent-runner/src/escalation.test.ts
echo "  → escalation module + tests in place"
...
# bun deps may be absent on a freshly composed tree (the host install does not
# populate them), and without bun-types the typecheck fails with a misleading
# TS2688 that looks like this skill broke something. Install first if needed.
(cd container/agent-runner \
  && { [ -d node_modules/bun-types ] || bun install --silent; } \
  && bun run typecheck \
  && bun test src/escalation.test.ts src/poll-loop.test.ts 2>&1 | tail -3)
