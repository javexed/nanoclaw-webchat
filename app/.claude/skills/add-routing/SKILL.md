...
- **Escalation is capped.** A backend that is simply down fails *every* turn,
  and each failure would otherwise bill the fallback. After
  `NANOCLAW_ESCALATION_CAP` consecutive escalations (default **3**) the
  handler declines and the primary's real error surfaces instead. The first
  delivered turn restores the budget. `NANOCLAW_ESCALATION_CAP=0` disables
  escalation without unsetting `fallback_provider`.
...
`container_configs.fallback_provider` column, the `ncl` flag, the
container-runner env/mount merge for the fallback provider). The escalation
logic itself is **not** a core patch: `escalation.ts` registers a turn-retry
handler (`runner-hooks.ts` R5), which is why the old ~154-line poll-loop
patch is gone — core owns the retry lifecycle, the module picks the provider
and owns the cap. Re-running the installer is idempotent; upstream drift is absorbed
...
### Context guard (deterministic, post-classification)

`bind-routes` also stamps each bound route with `max_prompt_tokens` — the
model's *configured* `num_ctx` (Ollama api/show; 4096 default) halved, since
Ollama reserves ~half the window for generation and **silently truncates** a
longer prompt. On every live request the hook estimates the request's tokens
(~4 chars/token over all messages) and, when the classified binding can't fit
it, reroutes to the bound route with the largest sufficient window — or, when
nothing local fits, escalates (`no_adequate_model … ctx:`), because delivering
to a model that can't see the conversation is worse than escalating. A long
conversation escalates on its own and comes back local after compaction. The
guard is inert when annotations are absent (old routes.json) and never fails a
request (`ctx_guard_error` in the log, classified target kept). Decisions log
`est_tokens` and `ctx_guard` in routing-shadow.jsonl. Skip annotation with
`--no-ctx`; point at a non-default Ollama with `--ollama <url>`.
