...
| File | What diverged | Coverage |
|---|---|---|
| `.github/workflows/ci.yml` | self-hosted-runner fixes: no `setup-node` (broken there), no `format:check` (upstream base doesn't hold the standard), gate `channels-webchat`, push-after-merge run, concurrency | `.github/*` class |
| `.github/workflows/label-pr.yml` | GitHub-only guard (`github-script` 404s on other runners) | `.github/*` class |
...
| `src/templates/create-agent.test.ts` | `testTimeout: 30_000` — the self-hosted runner starves the 5 s default under the full suite; restoring would reintroduce a flaky CI gate | **new** narrow coverage entry |
