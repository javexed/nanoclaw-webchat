...
async function renderWizardAccess() {
  ...
  const localhostOnly = !!(info && info.loopback) && !bearerOn && !tsHealthy && !proxyOn;
...
// Fire the deferred host restart at Finish (the token / network bind take effect
// on boot). The connection drops; the client reconnects on its own.
async function wizardTriggerRestart() {
  try {
    await authFetch('/api/webchat/restart', { method: 'POST', headers: { 'X-Webchat-CSRF': '1' } });
    showToast('Restarting to apply the bearer token — sign back in with the token you saved.', {
      kind: 'info',
      timeout: 15000,
    });
  } catch {
    /* the socket may already be dropping as the host restarts */
  }
}
function wireWizard() {
...
async function saveToolSecret(scope = null, p = '#secret') {
  ...
  // inferred from the host on the server — one place, not a dropdown.
  ...
  try {
    const r = await authFetch(toolSecretUrl(scope), {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-Webchat-CSRF': '1' },
      body: JSON.stringify({ value, hostPattern }),
    });
    if (!r.ok) {
